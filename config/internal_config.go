...
func (config *InternalConfig) registerCommands() {
	...
	config.addCommand("helm-kubernetes-dashboard", Labels{utils.NODE_BOOTSTRAPPER}, OS{}, fmt.Sprintf("%s get svc kubernetes-dashboard -n kube-system || %s install stable/kubernetes-dashboard --name kubernetes-dashboard --set=service.type=NodePort,service.nodePort=%d --set=image.tag=v%s --namespace kube-system", kubectlCommand, helmCommand, config.Config.DashboardPort, config.Config.Versions.KubernetesDashboard))
	config.addCommand("helm-cert-manager-setup", Labels{utils.NODE_BOOTSTRAPPER}, OS{}, fmt.Sprintf("%s list -q | grep cert-manager || %s install --name cert-manager --namespace kube-system stable/cert-manager --set=image.tag=v%s", helmCommand, helmCommand, config.Config.Versions.CertManager))
