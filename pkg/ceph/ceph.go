...
func (ceph *Ceph) RunSetup(dashboardUsername, dashboardPassword, radosgwUsername, radosgwPassword, sslCertificate, sslKey string, placementGroups, expectedNumberOfObjects uint) error {
	...
	commands := []string{
		...
		fmt.Sprintf("echo \"%s\" | %s dashboard set-rgw-api-access-key -i -", radosgwUsername, cephBinary),
		fmt.Sprintf("echo \"%s\" | %s dashboard set-rgw-api-secret-key -i -", radosgwPassword, cephBinary),
